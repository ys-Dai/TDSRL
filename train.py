...
from compute_F1 import f1_score

from utils.contrast_loss import cosine_similarity_loss

from utils.frequency_degradation import F_banks, F_Components_Scaling
...
def main(options):
    ...
    model = get_anomaly_transformer(input_d_data=d_data, 
                                    ...
                                    aug_window = aug_window).to(device)
    ...
    log_dir = os.path.join(config.LOG_DIR, time.strftime('%y%m%d%H%M%S_'+options.dataset+options.savename, time.localtime(time.time())))
    os.mkdir(log_dir)
...
#         train_loss = lambda pred, gt: -(gt * torch.log(pred + 1e-8) + (1 - gt.bool().float()) * torch.log(1 - pred + 1e-8)).mean()
    ...
    ## FD probability
    FD_Low_bank_prob = 1 - options.FD_Low_bank # 1-0.35=0.65 
    FD_High_bank_prob = FD_Low_bank_prob - options.FD_High_bank # 0.65-0.3=0.35  
    FD_Mixture_bank_prob = FD_High_bank_prob - options.FD_Mixture_bank # 0.35-0.2=0.15 
    FD_Component_Scaling_prob = FD_Mixture_bank_prob - options.FD_Component_Scaling # 0.15-0.15=0
    ...
    for i in range(options.initial_iter, max_iters):
        ...
        replacing_lengths = [aug_window for i in range(n_batch)]
        ...
        target_index = np.random.randint(aug_window,data_seq_len-2*replacing_lengths+1)

        # Replacing types and dimensions
        replacing_type_TF = np.random.uniform(0., 1., size=(n_batch,))  
        TA_replacing_prob = options.TA_replacing

        replacing_type = np.random.uniform(0., 1., size=(n_batch,))   
        replacing_type_F = np.random.uniform(0., 1., size=(n_batch,))   
        ...
        slices = [] # local adjacent segments

        # Create anomaly intervals.
        for j, rep, tar, leng, typ, typ_F, typ_TF, dim_num, dim_cat in zip(range(n_batch), replacing_index, target_index, replacing_lengths, replacing_type, replacing_type_F, replacing_type_TF, replacing_dim_numerical, replacing_dim_categorical):
            ...
            if leng > 0:
                ...
                if typ_TF < TA_replacing_prob: 
                 ...
                 # Frequence domain augmentation 
                else:
                    # FD_Low_bank
                    if typ_F > FD_Low_bank_prob:
                        _x_FA = F_banks(x = _x, channels = target_column_numerical, bank='low', num_perturbation_position = options.num_FD_perturbation_positions,\
                                            enlarge_min = options.FD_enlarge_min, enlarge_max = options.FD_enlarge_max, device=device)
                        
                        x_anomaly[j, tar:tar+leng] = 1
                        x[j][tar:tar+leng] = _x_FA.transpose(0, 1)
                    
                    # FD_High_bank
                    elif typ_F > FD_High_bank_prob:
                        _x_FA = F_banks(x = _x, channels = target_column_numerical, bank='high', num_perturbation_position = options.num_FD_perturbation_positions,\
                                            enlarge_min = options.FD_enlarge_min, enlarge_max = options.FD_enlarge_max, device=device)
                    ...
                    # FD_Mixture_bank
                    elif typ_F > FD_Mixture_bank_prob:
                        _x_FA = F_banks(x = _x, channels = target_column_numerical, bank='mixture', num_perturbation_position = options.num_FD_perturbation_positions,\
                                            enlarge_min = options.FD_enlarge_min, enlarge_max = options.FD_enlarge_max, device=device)
                        
                        x_anomaly[j, tar:tar+leng] = 1
                        x[j][tar:tar+leng] = _x_FA.transpose(0, 1)

                    # F_Components_Scaling
                    elif typ_F > FD_Component_Scaling_prob:
                        _x_FA = F_Components_Scaling(x = _x, channels = target_column_numerical, device=device)
                ...
                if tar-leng<0 or tar+2*leng>len(x[j]):
                    print("not long enough:", test_i, " ;  iteration:  ", i)
                    test_i = test_i+1

                    lenn = min(tar,len(x[j])-tar-2*leng)  
                    s2 = x[j][tar:tar+lenn]
                    s1 = x[j][tar-lenn:tar]  
                    s3 = x[j][tar+lenn:tar+2*lenn] 
                else:
                    s1 = x[j][tar-leng:tar] 
                    s2 = x[j][tar:tar+leng] 
                    s3 = x[j][tar+leng:tar+2*leng]  


                slice_new = torch.stack((s1,s2,s3),dim=0)  
                slices.append(slice_new)
        ...
        x1 = model(slices[0,...],contrast=True) 
        x2 = model(slices[1,...],contrast=True)
        x3 = model(slices[2,...],contrast=True)
        loss_contrast = cosine_similarity_loss(x1,x2,x3)
        ...
        if i % options.summary_steps == 0:
            with torch.no_grad():
             ...
             # WO PA 
                    ...
                    print(f'iteration: {i} | loss: {loss.item():.10f} | loss_contrast: {loss_contrast.item():.10f} | train accuracy: {acc:.10f}')
                    print(f'WO PA -- anomaly rate: {best_rate:.3f} | precision: {best_eval[0]:.5f} | recall: {best_eval[1]:.5f} | F1-score: {best_eval[2]:.5f}\n')

            # with PA
                    best_eval = (0, 0, 0)
                    best_rate = 0
                    for rate in np.arange(0.001, 0.301, 0.001):
                        evaluation = f1_score(test_label, estimation, rate, True, False)
                        if evaluation[2] > best_eval[2]:
                            best_eval = evaluation
                            best_rate = rate
                    
                    print(f'PA -- anomaly rate: {best_rate:.3f} | precision: {best_eval[0]:.5f} | recall: {best_eval[1]:.5f} | F1-score: {best_eval[2]:.5f}\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--summary_steps", default=100, type=int, help='steps for summarizing and saving of training log')
    ...
    parser.add_argument("--dataset", default='SWaT', type=str, help='SMAP/SMD/SWaT/NeurIPS')  
    ...
    parser.add_argument("--savename", default='test', type=str, help='the name of results file')  

    parser.add_argument("--batch_size", default=16, type=int)
    parser.add_argument("--n_features", default=512, type=int, help='number of features for a window')
    parser.add_argument("--patch_size", default=2, type=int, help='number of data points in a patch')
    ...
    parser.add_argument("--TA_replacing", default=0.35, type=float, help='probability for time domain augmentation') 

    parser.add_argument("--FD_Low_bank", default=0.15, type=float, help='probability for FD_Low-bank perturbation')
    parser.add_argument("--FD_High_bank", default=0.20, type=float, help='probability for FD_High-bank perturbation')
    parser.add_argument("--FD_Mixture_bank", default=0.30, type=float, help='probability for FD_Mixture-bank perturbation')
    parser.add_argument("--FD_Component_Scaling", default=0.35, type=float, help='probability FD_for Components Scaling')
    parser.add_argument("--num_FD_perturbation_positions", default=5, type=int, help='number of FD perturbation positions')
    parser.add_argument("--FD_enlarge_min", default=0.8, type=float, help='the minimum scaling range of FD new perturbation component')
    parser.add_argument("--FD_enlarge_max", default=1.6, type=float, help='the maximum scaling range of FD new perturbation component')
    ...
    parser.add_argument("--window_sliding", default=16, type=int, help='sliding steps of windocws for validation')
